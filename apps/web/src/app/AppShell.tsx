...
import { AppMenuBar, CommandPalette, CommandProvider, EditableContextMenu, commandShortcuts, createCommandRegistry, isEditableShortcutTarget, matchesShortcut, type AppCommandContext, type Command } from "../commands";
...
export function AppShell() {
  ...
  const [editableMenu, setEditableMenu] = useState<{ x: number; y: number; target: HTMLInputElement | HTMLTextAreaElement }>();
  ...
  async function bootstrap() {
    setBusy(true);
    try {
      const { stored, scanned } = await loadBootstrapDataWithRetry((attempt, maxAttempts) => {
      ...
      const localSelected = localStorage.getItem("taptap.selectedProjectId") ?? "";
      const nextSelected = stored.selectedProjectId || (nextProjects.some((project) => project.id === localSelected) ? localSelected : "");
      if (nextSelected) {
        setSelectedProjectId(nextSelected);
        setActiveModule((current) => current === "home" ? DEFAULT_PROJECT_MODULE : current);
      } else {
      ...
      setNotice(`本地 API 仍未就绪，请确认 Fastify 服务已启动：${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function confirmAssetReferenceMutation(projectId: string, relativePaths: string[], actionLabel: string) {
    const results = await scanAssetReferences(projectId, relativePaths);
    const referencedResults = results.filter((result) => result.referenceCount > 0);
    if (!referencedResults.length) return true;
    return window.confirm(buildAssetReferenceConfirmationMessage(actionLabel, referencedResults));
  }
  ...
  async function handleDeleteAssets(relativePaths: string[]) {
    ...
    setBusy(true);
    try {
      const confirmed = await confirmAssetReferenceMutation(selectedProject.id, relativePaths, "删除");
      if (!confirmed) {
        setNotice("已取消删除资产");
        return;
      }
  ...
  async function handleMoveAssets(relativePaths: string[], targetFolder: string) {
    ...
    setBusy(true);
    try {
      const confirmed = await confirmAssetReferenceMutation(selectedProject.id, relativePaths, "移动");
      if (!confirmed) {
        setNotice("已取消移动资产");
        return;
      }
  ...
  async function handleRenameAsset(relativePath: string, newName: string) {
    ...
    setBusy(true);
    try {
      const confirmed = await confirmAssetReferenceMutation(selectedProject.id, [relativePath], "重命名");
      if (!confirmed) {
        setNotice("已取消重命名资产");
        return;
      }
  ...
  const commandContext: AppCommandContext = useMemo(() => {
    if (selection?.type === "asset") return { objectType: "asset", relativePath: selection.item.relativePath };
    if (selection?.type === "task") return { objectType: "task", taskId: selection.item.taskId };
    if (selection?.type === "tool") return { objectType: "mcpTool", toolName: selection.item.name };
    if (selection?.type === "project") return { objectType: "project", projectId: selection.item.id };
    if (activeModule === "workflow") return { objectType: "workflowCanvas" };
    if (selectedProject) return { objectType: "project", projectId: selectedProject.id };
  ...
  const commands = useMemo<Command[]>(() => [
    ...
    },
    {
      ...
      scope: "global",
      run: () => selectedProject ? void refreshProject(selectedProject.id) : void handleScanProjects()
    },
    {
      ...
      run: () => {
        if (selection?.type === "asset") {
          void copyText(selection.item.relativePath, { successMessage: "资产路径已复制" });
          return;
        }
        if (selection?.type === "task") {
          void copyText(getTaskCopyPayload(selection.item), { successMessage: "任务内容已复制" });
          return;
        }
        if (selection?.type === "tool") {
          void copyText(selection.item.name, { successMessage: "工具名已复制" });
          return;
        }
        if (selection?.type === "project") void copyText(selection.item.rootPath, { successMessage: "项目路径已复制" });
      }
    },
    {
      ...
      run: () => {
        if (selection?.type === "asset") {
          const confirmed = window.confirm(`确认删除资产？\n\n${selection.item.relativePath}`);
          if (confirmed) void handleDeleteAssets([selection.item.relativePath]);
          return;
        }
        if (selection?.type === "task") {
          const confirmed = window.confirm(`确认删除任务记录？\n\n${selection.item.taskId}`);
    ...
    },
    {
      ...
      title: "保存当前工作流/草稿",
      description: activeModule === "workflow" ? "保存当前节点流" : "当前面板没有可保存草稿",
    ...
    },
    {
      ...
      title: "执行当前生成/节点",
      description: activeModule === "workflow" ? "运行当前节点流中已配置节点" : "当前面板执行入口仍在具体工作室内",
    ...
    },
    {
      ...
      run: () => selectedProject ? void copyText(selectedProject.rootPath, { successMessage: "项目路径已复制" }) : undefined
    },
    {
      ...
      scope: "project",
      when: () => !!selectedProject,
      run: () => selectedProject ? void copyText(selectedProject.makerProjectId, { successMessage: "project_id 已复制" }) : undefined
    },
    {
    ...
    },
    {
      ...
      scope: "global",
      run: () => void copyText(buildDiagnosticSummary({ selectedProject, runtime, tools, assets, tasks, activeModule }), { successMessage: "诊断摘要已复制" })
    },
    {
    ...
    },
    {
      ...
      scope: "project",
      when: () => !!selectedProject,
      run: () => selectedProject ? void refreshProject(selectedProject.id) : undefined
    },
    {
    ...
    },
    {
      ...
      scope: "asset",
      when: (context) => context.objectType === "asset" && assets.some((asset) => asset.relativePath === context.relativePath),
      run: (context) => {
        if (context.objectType !== "asset") return;
        const asset = assets.find((item) => item.relativePath === context.relativePath);
    ...
    },
    {
      ...
      run: (context) => {
        if (context.objectType === "asset") void copyText(context.relativePath, { successMessage: "资产路径已复制" });
      }
    },
    {
    ...
    },
    {
      ...
      scope: "task",
      when: (context) => context.objectType === "task" && tasks.some((task) => task.taskId === context.taskId),
      run: (context) => {
    ...
    },
    {
      ...
      run: (context) => {
        if (context.objectType !== "task") return;
        const task = tasks.find((item) => item.taskId === context.taskId);
        if (task) void copyText(getTaskCopyPayload(task), { successMessage: "任务 raw/error 已复制" });
      }
    },
    {
      ...
      scope: "mcpTool",
      when: (context) => context.objectType === "mcpTool" && tools.some((tool) => tool.name === context.toolName),
      run: (context) => {
    ...
    {
      commandId: "workflowNode.copyNodeId",
      title: "复制节点 ID",
      description: "复制当前工作流节点 ID",
      scope: "workflowNode",
      when: (context) => context.objectType === "workflowNode",
      run: (context) => {
        if (context.objectType === "workflowNode") void copyText(context.nodeId, { successMessage: "节点 ID 已复制" });
      }
    }
  ], [activeModule, assets, inspectorMinimized, runtime, selectedProject, selection, sidebarCollapsed, tasks, theme, tools]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      for (const command of commandRegistry.list(commandContext)) {
        if (!commandShortcuts(command.shortcut, command.shortcuts).some((shortcut) => matchesShortcut(event, shortcut))) continue;
        event.preventDefault();
  ...
  useEffect(() => {
    const onContextMenu = (event: MouseEvent) => {
      ...
      if (editableTarget) {
        ...
        setEditableMenu({ x: event.clientX, y: event.clientY, target: editableTarget });
        return;
      }
    ...
    const stopBrowserShortcuts = (event: KeyboardEvent) => {
      ...
      if (
        key === "f12"
        || key === "f5"
        || ((event.ctrlKey || event.metaKey) && (key === "f" || key === "o" || key === "p" || key === "r" || key === "s" || key === "u"))
        || ((event.ctrlKey || event.metaKey) && event.shiftKey && (key === "c" || key === "i" || key === "j"))
      ) {
        event.preventDefault();
      }
    };
    ...
    return () => {
      ...
      window.removeEventListener("dragover", stopNavigationDrop, { capture: true });
      window.removeEventListener("drop", stopNavigationDrop, { capture: true });
      window.removeEventListener("keydown", stopBrowserShortcuts, { capture: true });
    };
  }, []);

  async function beginDesktopAspectResize(event: React.PointerEvent<HTMLDivElement>, edge: ResizeEdge) {
    ...
    try {
      ...
      const resizeTo = (width: number, height: number) => {
        ...
        animationFrame = window.requestAnimationFrame(() => {
          void appWindow.setSize(new PhysicalSize(Math.round(width), Math.round(height)));
        });
      };
      const onMove = (moveEvent: PointerEvent) => {
        ...
        if (edge === "e") {
          nextWidth = Math.max(DESKTOP_MIN_WIDTH, startSize.width + deltaX);
          nextHeight = Math.max(DESKTOP_MIN_HEIGHT, nextWidth * DESKTOP_ASPECT_HEIGHT / DESKTOP_ASPECT_WIDTH);
        } else if (edge === "s") {
          nextHeight = Math.max(DESKTOP_MIN_HEIGHT, startSize.height + deltaY);
          nextWidth = Math.max(DESKTOP_MIN_WIDTH, nextHeight * DESKTOP_ASPECT_WIDTH / DESKTOP_ASPECT_HEIGHT);
        } else {
          const widthFromX = Math.max(DESKTOP_MIN_WIDTH, startSize.width + deltaX);
          const heightFromY = Math.max(DESKTOP_MIN_HEIGHT, startSize.height + deltaY);
          if (Math.abs(deltaX) >= Math.abs(deltaY)) {
            nextWidth = widthFromX;
            nextHeight = nextWidth * DESKTOP_ASPECT_HEIGHT / DESKTOP_ASPECT_WIDTH;
          } else {
            nextHeight = heightFromY;
            nextWidth = nextHeight * DESKTOP_ASPECT_WIDTH / DESKTOP_ASPECT_HEIGHT;
          }
        }
  ...
  const fallbackCommands = commandRegistry.list(commandContext).filter((command) => !command.commandId.startsWith("app.copyCurrent") || !!selection);
    ...
    <CommandProvider registry={commandRegistry}>
      ...
      >
        <div className="relative shrink-0 h-full" style={{ width: sidebarCollapsed ? 56 : sidebarWidth }}>
          <ProjectSidebar projects={projects} selectedProjectId={selectedProjectId} activeModule={activeModule} tasks={tasks} collapsed={sidebarCollapsed} width={sidebarWidth} onToggleCollapsed={() => setSidebarCollapsed((value) => !value)} onClearProject={handleClearProject} onSelectModule={selectModule} onScanProjects={handleScanProjects} />
        ...
        <div className="relative shrink-0 flex h-full border-l border-border bg-surface-panel" style={{ width: "var(--inspector-width)", transition: "width 0.3s cubic-bezier(0.16, 1, 0.3, 1)" }}>
          ...
          {fallbackMenu ? (
            ...
            >
              <div className="px-3 py-2.5 text-[11px] font-bold uppercase tracking-widest text-text-muted/60">{commandContext.objectType} Actions</div>
              <div className="mb-1.5 mx-2 h-px bg-border/50" />
              {fallbackCommands.length ? fallbackCommands.map((command) => (
                <button
                ...
                <div className="px-3 py-2.5 text-xs text-text-muted">暂无可用命令</div>
              )}
            </div>
          ) : null}
          <EditableContextMenu menu={editableMenu} onClose={() => setEditableMenu(undefined)} />
        </div>
    </CommandProvider>
  );
}
...
async function loadBootstrapDataWithRetry(onRetry: (attempt: number, maxAttempts: number) => void) {
...
function getTaskCopyPayload(task: TaskRecord) {
  return task.rawResultJson || task.errorMessage || JSON.stringify(task, null, 2);
}

function getEditableMenuTarget(target: EventTarget | null): HTMLInputElement | HTMLTextAreaElement | undefined {
...
function DesktopAspectResizeHandles({ onResizeStart }: { onResizeStart: (event: React.PointerEvent<HTMLDivElement>, edge: ResizeEdge) => void }) {
  return (
    <>
      <div className="fixed bottom-0 right-2 top-14 z-[70] w-2 cursor-ew-resize" data-no-window-drag onPointerDown={(event) => onResizeStart(event, "e")} />
      <div className="fixed bottom-0 left-2 right-2 z-[70] h-2 cursor-ns-resize" data-no-window-drag onPointerDown={(event) => onResizeStart(event, "s")} />
      <div className="fixed bottom-0 right-0 z-[71] h-5 w-5 cursor-nwse-resize" data-no-window-drag onPointerDown={(event) => onResizeStart(event, "se")} />
    </>
  );
}
...
function toAgentSelectionReference(selection: InspectorSelection): AgentSelectionReference | undefined {
  if (!selection) return undefined;
  if (selection.type === "project") return { type: "project", projectId: selection.item.id };
  if (selection.type === "tool") return { type: "tool", toolName: selection.item.name };
  if (selection.type === "task") return { type: "task", taskId: selection.item.taskId };
  return { type: "asset", relativePath: selection.item.relativePath };
}

function buildAssetReferenceConfirmationMessage(actionLabel: string, results: AssetReferenceScanResult[]) {
  const lines = results.flatMap((result) => [
    `${result.relativePath}：${result.referenceCount} 处引用`,
    ...result.references.slice(0, 5).map((reference) => `  - ${reference.sourcePath}:${reference.line}:${reference.column} ${reference.lineText.trim()}`)
  ]);
