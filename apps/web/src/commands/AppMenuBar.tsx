      ...
      {sections.map((section) => {
        ...
        return (
          <div key={section.label} className="relative">
            ...
            {openSection === section.label ? (
              <div className="absolute left-0 top-full mt-1.5 z-50 min-w-[260px] overflow-hidden rounded-xl border border-border/60 bg-surface-panel/95 backdrop-blur-xl p-1.5 shadow-[0_16px_40px_-10px_rgba(0,0,0,0.3)] ring-1 ring-white/5 origin-top-left animate-in fade-in zoom-in-95 duration-100">
                {commands.map((command) => (
                  <button
                    ...
                    className={cn(
                      "group flex w-full cursor-pointer select-none items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[13px] outline-none transition-all duration-200",
                      command.danger 
