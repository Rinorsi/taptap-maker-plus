...
import {
  ...
  StudioPromptField,
  StudioSelectField,
} from "../../components/studio/StudioKit";
import { AssetManagerPanel } from "../assets/AssetManagerPanel";
import {
  defaultAssetImportFolders,
  managedAssetRoots,
...
function readVideoTaskId(rawResultJson: string) {
  try {
    const raw = JSON.parse(rawResultJson) as unknown;
    return findVideoTaskId(raw);
  } catch {
    const match = rawResultJson.match(/cgt-[a-zA-Z0-9-]+/);
    return match ? match[0] : undefined;
  }
}

function findVideoTaskId(value: unknown): string | undefined {
  if (typeof value === "string") {
    const match = value.match(/cgt-[a-zA-Z0-9-]+/);
    return match ? match[0] : undefined;
  }
  if (Array.isArray(value)) {
    for (const item of value) {
      const taskId = findVideoTaskId(item);
  ...
  for (const key of ["task_id", "taskId", "id"]) {
    const taskId = findVideoTaskId(object[key]);
    if (taskId) return taskId;
  }
...
export function VideoStudio({
  ...
  const currentRoot =
    resourceType === "video"
      ? managedAssetRoots.video
      : resourceType === "image"
        ? managedAssetRoots.image
        : managedAssetRoots.audio;
  const currentImportFolder =
    resourceType === "video"
      ? defaultAssetImportFolders.video
      : resourceType === "image"
        ? defaultAssetImportFolders.image
        : defaultAssetImportFolders.audio;
  ...
  const latestCloudVideoTask = useMemo(() => {
    if (!project) return undefined;
    const videoTasks = tasks.filter(
      (t) =>
        t.projectId === project.id &&
        (t.toolName === "create_video_task" ||
          t.toolName === "query_video_task"),
    );
    const latestCompleted = videoTasks.find(
      (t) => t.status === "succeeded" && t.rawResultJson,
    );
    return latestCompleted;
  }, [project, tasks]);

  const latestCloudVideoTaskId = useMemo(() => {
    if (!latestCloudVideoTask?.rawResultJson) return undefined;
    return readVideoTaskId(latestCloudVideoTask.rawResultJson);
  }, [latestCloudVideoTask]);

  const isCloudVideoRunning = useMemo(() => {
    if (!latestCloudVideoTask?.rawResultJson) return false;
    return isVideoTaskStatusRunning(latestCloudVideoTask.rawResultJson);
  }, [latestCloudVideoTask]);

  const cloudVideoQueryTimerRef = useRef<number | undefined>(undefined);
  useEffect(() => {
    window.clearTimeout(cloudVideoQueryTimerRef.current);
    if (!project || !isCloudVideoRunning || !latestCloudVideoTaskId) return;

    const taskTime =
      new Date(
        latestCloudVideoTask?.finishedAt ??
          latestCloudVideoTask?.startedAt ??
          Date.now(),
      ).getTime() || Date.now();
    const delay = Math.max(
      0,
      taskTime + videoTaskQueryIntervalMs - Date.now(),
    );

    cloudVideoQueryTimerRef.current = window.setTimeout(() => {
      void onCallTool("query_video_task", {
        task_id: latestCloudVideoTaskId,
      }).then(() => {
        onScanAssets();
      });
    }, delay);

    return () => window.clearTimeout(cloudVideoQueryTimerRef.current);
  }, [
    project,
    isCloudVideoRunning,
    latestCloudVideoTask,
    latestCloudVideoTaskId,
    onCallTool,
    onScanAssets,
  ]);
  ...
  const previewMetadata = useMemo(() => {
    ...
    const task = tasks.find(
      (t) =>
        t.status === "succeeded" &&
        t.rawResultJson &&
        t.rawResultJson.includes(displayAsset.fileName),
    );
    if (!task) return null;
    try {
  ...
  function handleGenerate() {
    ...
    if (videoMode === "first_frame" && firstFrame) {
      ...
      videoMode === "multi_modal_reference"
    ) {
    ...
    >
      ...
      <div className="flex-1 flex gap-5 min-h-0 overflow-hidden">
        {studioMode === "standard" ? (
            ...
            <div className="relative z-10 flex flex-col h-full min-h-0 overflow-hidden rounded-3xl">
              ...
              <div className="flex items-center justify-between border-b border-white/5 pb-4 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-black text-text uppercase tracking-widest">
                    导演控制台
                  </span>
                </div>
                ...
                <div className="flex gap-3">
                  <div className="flex-1 flex flex-col gap-2">
                    ...
                    <StudioMediaDropzone
                      image={
                        firstFrame
                          ? assetPreviewUrl(project?.id ?? "", firstFrame)
                          : ""
                      }
                      onChange={setFirstFrame}
                      height="h-28"
                      emptyLabel="拖拽图片至此"
                      emptySubLabel="作为首帧参考"
                    />
                  </div>
                  ...
                  {(videoMode === "first_last_frame" ||
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                      ...
                      <StudioMediaDropzone
                        image={
                          lastFrame
                            ? assetPreviewUrl(project?.id ?? "", lastFrame)
                            : ""
                        }
                        onChange={setLastFrame}
                        height="h-28"
                        emptyLabel="拖拽图片至此"
                        emptySubLabel="作为尾帧参考"
                      />
                    </div>
                  )}
                </div>
              ...
              <div className="pt-2">
                <button
                  ...
                  className={cn(
                    "flex items-center justify-center gap-2 text-[13px] font-bold transition-all w-full py-3 rounded-2xl group",
                    showAdvanced
                      ? "bg-brand/10 text-brand"
                      : "bg-surface-raised text-text-muted hover:bg-surface-panel hover:text-text",
                  )}
                >
                  <Settings2
                    className={cn(
                    ...
                    >
                      <div className="grid grid-cols-2 gap-5 pt-5 pb-2">
                        <div className="col-span-1 flex flex-col gap-2">
                          <Label
                            htmlFor="name"
                            className="text-xs font-bold text-text-subtle"
                          >
                            视频名称 (可选)
                          </Label>
                          <Input
                        ...
                        <div className="col-span-2 flex flex-col gap-4 pt-2">
                          <label className="flex items-center gap-3 cursor-pointer group">
                            ...
                            <span className="text-xs font-bold text-text-muted group-hover:text-text transition-colors">
                              生成有声视频 (Generate Audio)
                            </span>
                          </label>
                          ...
                          <label className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative">
                              <input
                                ...
                                onChange={(e) =>
                                  setReturnLastFrame(e.target.checked)
                                }
                              />
                              <div className="w-9 h-5 bg-surface-raised peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-brand"></div>
                            </div>
                            <span className="text-xs font-bold text-text-muted group-hover:text-text transition-colors">
                              返回尾帧图 (Return Last Frame)
                            </span>
                          </label>
          ...
          <div
            className={cn(
              "flex-1 min-w-0 min-h-0 overflow-hidden relative",
              isFlowFullscreen
                ? "bg-surface-app rounded-none border-0 shadow-none"
                : "bg-surface-panel/40 border border-white/5 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.12)]",
            )}
          >
            <VideoFlowCanvas
              project={project}
                  ...
                  <div className="flex flex-wrap items-center gap-3 min-w-0 flex-1">
                    <div className="flex p-1 bg-surface-app border border-border-soft rounded-lg shadow-inner shrink-0">
                      ...
                      <button
                        ...
                        className={cn(
                          "px-3 py-1 text-[11px] font-bold rounded transition-all",
                          viewTab === "manage" && resourceType === "video"
                            ? "bg-surface-raised shadow-sm text-text border border-border-soft"
                            : "text-text-subtle hover:text-text",
                        )}
                      >
                      ...
                      <button
                        ...
                        className={cn(
                          "px-3 py-1 text-[11px] font-bold rounded transition-all",
                          viewTab === "manage" && resourceType === "image"
                            ? "bg-surface-raised shadow-sm text-text border border-border-soft"
                            : "text-text-subtle hover:text-text",
                        )}
                      >
                      ...
                      <button
                        ...
                        className={cn(
                          "px-3 py-1 text-[11px] font-bold rounded transition-all",
                          viewTab === "manage" && resourceType === "audio"
                            ? "bg-surface-raised shadow-sm text-text border border-border-soft"
                            : "text-text-subtle hover:text-text",
                        )}
                      >
                  ...
                  <div className="flex flex-wrap items-center gap-3 shrink-0">
                    {studioMode !== "standard" && (
                      <button
                        ...
                        className="ml-2 p-1.5 rounded-full hover:bg-surface-app text-text-subtle hover:text-text transition-colors"
                        title="收起素材库"
                      >
                      ...
                      {activeGenerationTask ? (
                        ...
                        <div className="flex flex-col items-center max-w-full w-full mt-4">
                          <div className="relative mb-4 shrink-0">
                            <button
                              type="button"
                              className="group relative block overflow-hidden rounded-xl bg-black/20 shadow-2xl"
                              onClick={() => setPreviewAsset(displayAsset)}
                              title="打开视频预览"
                            >
                              <video
                                src={assetPreviewUrl(
                                  project?.id ?? "",
                                  displayAsset.relativePath,
                                )}
                                muted
                                playsInline
                                preload="metadata"
                              ...
                              <span className="absolute inset-0 flex items-center justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                                <span className="flex h-12 w-12 items-center justify-center rounded-full bg-black/55 text-white shadow-xl backdrop-blur-sm">
                          ...
                          <div className="w-full max-w-4xl shrink-0 bg-surface-raised/80 p-2.5 pr-3 rounded-xl border border-border flex items-center justify-between gap-4 text-[11px] text-text-subtle shadow-sm relative overflow-hidden">
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 pl-3 flex-1 min-w-0">
                              {previewMetadata?.prompt && (
                                <div className="flex items-center min-w-0">
                                  <span className="font-bold text-text-muted mr-1.5 shrink-0">
                                    Prompt:
                                  </span>
                                  <span
                                    className="text-text truncate max-w-[200px]"
                                    title={previewMetadata.prompt}
                                  >
                                    {previewMetadata.prompt}
                                  </span>
                                </div>
                              ...
                              <div className="flex items-center shrink-0">
                                <span className="font-bold text-text-muted mr-1.5">
                                  Model:
                                </span>
                                <span className="bg-surface-app px-1.5 py-0.5 rounded text-text">
                                  {previewMetadata?.model || "default"}
                                </span>
                              </div>
                            ...
                            <div className="shrink-0 pl-3 border-l border-white/5">
                              <Button
                                ...
                                onClick={() => {
                                  void deleteAssetsAndClearPreview([
                                    displayAsset.relativePath,
                                  ]);
                                  setPreviewAsset(null);
                                }}
          ...
          >
            {previewAsset.assetType === "video" ? (
              <video
                ...
                controls
                autoPlay
                className="max-h-[95vh] max-w-[95vw] rounded-xl bg-black object-contain shadow-2xl cursor-default"
